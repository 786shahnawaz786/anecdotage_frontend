...
function getItem(isPrev, goTo) {
  ...
  if (item < 1) {
    ...
    if (!isPrev && pageType != 'SingleFact') {
      GApageview(pageURL + '/Item' + item);
    }
  }

  //---- Load taboola if it was never loaded
  if (Taboola && !TaboolaLoaded) {
    loadTaboola();
  }
}
...
function share_getText() {
  ...
  return $.trim(
    itemOBJ
      ...
      .replace(/	/gm, ' ')
      .replace(/ +(?= )/g, '')
      .replace(itemOBJ.find('.factTools').text(), '')
  );
}
...
function share_getImage() {
  return (
    'https://www.factslides.com/~assets/images/ISHOTS/' + sharedItem + '.png'
  );
}
...
function share_TW() {
  var url = share_getURL();
  var purl =
    'https://twitter.com/intent/tweet?hashtags=fact&text=' +
    encodeURIComponent(share_getText()) +
    '&url=' +
  ...
  var pwin = window.open(
    purl,
    'twittershare',
    'width=' + pw + ',height=' + ph + ',top=' + pt + ',left=' + pl
  );
...
function share_PI() {
  ...
  var purl =
    ...
    encodeURIComponent(url) +
    '&media=' +
    encodeURIComponent(share_getImage()) +
    '&description=' +
    encodeURIComponent(share_getText());
  var pw = 780;
  var ph = 600;
...
function share_EM() {
  ...
  location =
    'mailto:?subject=' +
    encodeURIComponent('Interesting Fact') +
    '&body=' +
    encodeURIComponent(share_getText()) +
    ' - Read more at: ' +
    encodeURIComponent(url);
...
function logo_toggle() {
  if (UTIL_obj('logo_menu').style.display == 'none') {
    logo_showMenu();
  } else {
    logo_hideMenu();
  }
...
function loadTaboola() {
  ...
  !(function(e, f, u) {
    ...
    '//cdn.taboola.com/libtrc/oddee-factslides/loader.js'
  );
}
...
function GApageview(pageurl) {
  ...
  if (!pISLOCAL && !pISTEST) {
    ...
    if (GAshowalert) {
      alert('Google Analytics Pageview: ' + pageurl);
    }
  }
}
...
function UTIL_sWin(p) {
  window.open(
    p,
    'sWin',
    'width=700,height=450,top=100,left=100,menubar=no,titlebar=no,toolbar=no'
  );
  return false;
}
