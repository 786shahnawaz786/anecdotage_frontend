        ...
        <div id="navbar" class="collapse navbar-collapse">
          <ul class="navbar-nav font-14 fw-300">
            <li class="nav-item dropdown navbar-menu-item">
              <a id="menuLink" aria-expanded="false" aria-haspopup="true" class="nav-link dropdown-toggle"
                data-toggle="dropdown" href="#">
                {{ routeName }}
                <svg aria-hidden="true" class="svg-inline--fa fa-caret-down" data-fa-i2svg="" data-icon="caret-down"
                  data-prefix="fas" data-v-6ea372ba="" focusable="false" role="img" viewBox="0 0 320 512"
              ...
              <div aria-labelledby="menuLink" class="dropdown-menu">
                <nuxt-link :to="{ name: 'threads.rated' }" class="dropdown-item">Top Rated
                </nuxt-link>
                <nuxt-link :to="{ name: 'threads.trending' }" class="dropdown-item">Trending
                </nuxt-link>
                <nuxt-link :to="{ name: 'threads.recent' }" class="dropdown-item">Most Recent
                </nuxt-link>
                <nuxt-link :to="{ name: 'threads.viewed' }" class="dropdown-item">Most Viewed
                </nuxt-link>

                <nuxt-link :to="{ name: 'threads.last.viewed' }" class="dropdown-item">Last Viewed
                </nuxt-link>
                <nuxt-link :to="{ name: 'threads.closest' }" class="dropdown-item">Closest
                </nuxt-link>




                <nuxt-link :to="{ name: 'listbooks' }" class="dropdown-item">Books
                </nuxt-link>
                ...
                > -->
                <nuxt-link v-show="isAdmin" :to="{ name: 'threads.sortbytitle.length' }" class="dropdown-item">Sort By
                  Title Length
                </nuxt-link>
            ...
            <li class="nav-item dropdown navbar-menu-item">
              <a id="channelLink" aria-expanded="false" aria-haspopup="true" class="nav-link dropdown-toggle"
              ...
              <div aria-labelledby="channelLink" class="dropdown-menu">
                <nuxt-link v-for="channel in channels" :key="channel.id" :class="{ 'active-tag': activeTag(channel) }"
                  :to="{ name: 'tags', params: { slug: channel.slug } }" class="dropdown-item">{{ channel.name }}
                </nuxt-link>
              </div>
            </li>

            <li class="nav-item dropdown navbar-menu-item">
              <a :style="{ 'background-image': `url(${defaultEmojiUrl})` }" aria-expanded="true" aria-haspopup="true"
                aria-label="browse by emoji" class="dropdown-toggle emoji-dropdown" data-toggle="dropdown" href="#"
                role="button" style="
                  width: 24px;
                  height: 24px;
                  display: block;
                ...
                <svg aria-hidden="true" class="svg-inline--fa fa-caret-down emoji-caret" data-fa-i2svg=""
                  data-icon="caret-down" data-prefix="fas" data-v-6ea372ba="" focusable="false" role="img"
              ...
              <div class="dropdown-menu">
                <nuxt-link v-for="emoji in emojis" :key="emoji.id" :style="backgroundEmoji(emoji.name)"
                  :to="{ name: 'emojis', params: { emoji: emoji.name } }"
                  class="navigation-emoji navigation-emoji-icon dropdown-item">{{ emoji.name }}
                </nuxt-link>
              </div>
            </li>
          </ul>
          <SearchForm></SearchForm>
          <div class="upload-shot white-path font-14 fw-500 text-uppercase d-flex">
            <nuxt-link :to="{ name: 'maps' }" aria-label="Maps" class="navbar-menu-icon" href="#">
              <img alt="" class="navbar-icon marker" data-placement="top" data-toggle="tooltip" height="24"
                src="~assets/images/map_marker.png" style="width:16px; height:24px" title="View Map" width="16" />
            </nuxt-link>
            <nuxt-link :to="{ name: 'threads.create' }" aria-label="Create New Thread" class="navbar-menu-icon">
              <img alt="" class="navbar-icon pen" data-placement="top" data-toggle="tooltip" height="24"
                src="~assets/images/pen_with.png" style="width:17px; height:24px" title="Add a Post" width="17%" />
            </nuxt-link>
          </div>
          <!-- <div
          ...
          <template v-if="!$auth.loggedIn">
            <ul class="before-login font-14 fw-300 text-uppercase">
              <li>
                <nuxt-link :to="{ name: 'login' }" class="btn btn-dark my-2 my-sm-0">Sign In
                </nuxt-link>
              </li>
              <li>
                <nuxt-link :to="{ name: 'register' }" class="btn btn-primary my-2 my-sm-0">Sign Up
                </nuxt-link>
              </li>
            </ul>
          </template>
          ...
          <template v-else>
            <ul class="author-page white-path">
              ...
              <!-- Profile Dropdown -->
              <li class="dropdown">
                <a id="userDropdown" aria-expanded="false" aria-haspopup="true" class="dropdown-toggle"
                  data-toggle="dropdown" role="button" style="color: black; font-size: 14px">
                  <img :alt="$auth.user.name" :src="$auth.user.photo_url" class="user-thumb" height="36"
                    style="width:36px; height:36px" width="36" />
                  <!-- {{$auth.user.name}} -->
                  Me
                  <span class="down-chevron" style="margin-left: 5px">
                    <svg aria-hidden="true" class="svg-inline--fa fa-caret-down" data-icon="caret-down"
                      data-prefix="fas" focusable="false" role="img" viewBox="0 0 320 512"
                      xmlns="http://www.w3.org/2000/svg">
                      <path
                        d="M31.3 192h257.3c17.8 0 26.7 21.5 14.1 34.1L174.1 354.8c-7.8 7.8-20.5 7.8-28.3 0L17.2 226.1C4.6 213.5 13.5 192 31.3 192z"
                        fill="currentColor"></path>
                ...
                <div aria-labelledby="userDropdown" class="dropdown-menu user-dropdown font-14 fw-500">
                  <nuxt-link :to="{
                    name: 'profile.show',
                    params: { username: $auth.user.username },
                  }" class="dropdown-item" href="#" title="Profile">
                  ...
                  <nuxt-link :to="{
                    ...
                    params: { username: $auth.user.username },
                  }" class="dropdown-item" href="#" title="Setting">
                  ...
                  <nuxt-link v-show="$auth.user.is_admin" :to="{ name: 'admin.show' }" class="dropdown-item" href="#"
                    title="Setting">
                    <svg aria-hidden="true" class="svg-inline--fa fa-cogs" data-icon="cogs" data-prefix="fas"
                      focusable="false" role="img" viewBox="0 0 640 512" xmlns="http://www.w3.org/2000/svg">
                      <path
