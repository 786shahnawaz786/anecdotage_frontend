<template>
  <div class="container">
    <div class="row">
      <div class="col-md-8">
        <div></div>
        <safe-search :postCounts="pageinateData.total" :totalThreadsCount="totalThreadsCount" :card="true"></safe-search></safe-search>
...
export default {
  ...
  head() {
    return {
      title: `${this.settings.site_title} | Last Viewed`,
    };
  },
  watchQuery: true,
  ...
  async fetch({ params, query, app, $axios, store }) {
    ...
    try {
      const threadRresponse = await $axios.$get(`threads/filter/last-modified?${q}`);
